...
double cap_right_real (sasfit_param *param )
{
    ...
    result = (-1.0)*DIF_ETA_SH*4.0*M_PI*R_SH*sin(THETA_1)*J1divQQ(u,
             param)*(sin(a) - sin(b));
    return result;
...
double cap_right_imag (sasfit_param *param )
{
    ...
    a = Q*R_SH*cos(THETA_1)*cos(PSI);
    b = Q*(R_SH + T_SH)*cos(BIG_THETA_1)*cos(PSI);
    result = DIF_ETA_SH*4.0*M_PI*R_SH*sin(THETA_1)*J1divQQ(u,
             param)*(cos(a) - cos(b));
    return result;
}
...
double cap_left_real (sasfit_param *param )
{
    ...
    result = (-1.0)*DIF_ETA_SH*4.0*M_PI*R_SH*sin(THETA_2)*J1divQQ(u,
             param)*(sin(a) - sin(b));
    return result;
}
...
double cap_left_imag (sasfit_param *param )
{
    ...
    a = (-1.0)*Q*(R_SH+T_SH)*cos(BIG_THETA_2)*cos(PSI);
    b = -(1.0)*Q*R_SH*cos(THETA_2)*cos(PSI);
    result = DIF_ETA_SH*4.0*M_PI*R_SH*sin(THETA_2)*J1divQQ(u,
             param)*(cos(a) - cos(b));
    return result;
}
...
double dom_cap_right_real (sasfit_param *param )
{
    ...
    b = Q*(R_SH+T_SH)*cos(BIG_THETA_1)*cos(PSI);
    result = (-1.0)*DIF_ETA_C*4.0*M_PI*R_SH*sin(THETA_1)*J1divQQ(u,
             param)*(sin(a) - sin(b));
    return result;
}
...
double dom_cap_right_imag (sasfit_param *param )
{
    ...
    a = Q*((R_SH+T_SH)*cos(BIG_THETA_1) - T_C)*cos(PSI);
    b = Q*(R_SH+T_SH)*cos(BIG_THETA_1)*cos(PSI);
    result = DIF_ETA_C*4.0*M_PI*R_SH*sin(THETA_1)*J1divQQ(u,
             param)*(cos(a) - cos(b));
    return result;
}
...
double dom_cap_left_real (sasfit_param *param )
{
    ...
    result = (-1.0)*DIF_ETA_C*4.0*R_SH*sin(THETA_2)*M_PI*J1divQQ(u,
             param)*(sin(a) - sin(b));
    return result;
}
...
double dom_cap_left_imag (sasfit_param *param )
{
    ...
    a = (-1.0)*Q*(R_SH*T_SH)*cos(BIG_THETA_2)*cos(PSI);
    b = (-1.0)*Q*((R_SH+T_SH)*cos(BIG_THETA_2)+T_C)*cos(PSI);
    result = DIF_ETA_C*4.0*M_PI*R_SH*sin(THETA_2)*J1divQQ(u,
             param)*(cos(a) - cos(b));
    return result;
}
...
/* ******************************************************************************* */
double sasfit_ff_magneticfieldalignedvesicle_psi_new ( double q,
        sasfit_param *param )
{
    double res, err;
    double xl[3], xu[3];
    ...
    status = sasfit_integrate_plugin_new(
                 &sasfit_ff_magneticfieldalignedvesicle_new_kernel, xl, xu, 3, param,
                 sasfit_eps_get_iter_4_mc(), &res, &err );
...
//	sasfit_out("r1a: %lf\n", r1a);
    ...
    else
    {
        r1b  = sasfit_integrate( (-1.0)*R_SH*cos(THETA_2), R_SH*cos(THETA_1),
                                 inner_sphere_imag, param );
    }
//	sasfit_out("r1b: %lf\n", r1b);
    if ( param->errStatus ) return 0.0;
    r2a  = sasfit_integrate( (-1.0)*(R_SH+T_SH)*cos(BIG_THETA_2),
                             (R_SH+T_SH)*cos(BIG_THETA_1), outer_sphere_real, param );
//	sasfit_out("r2a: %lf\n", r2a);
    if ( param->errStatus ) return 0.0;
    ...
    else
    {
        r2b  = sasfit_integrate( (-1)*(R_SH+T_SH)*cos(BIG_THETA_2),
                                 (R_SH+T_SH)*cos(BIG_THETA_1), outer_sphere_imag, param );
    }
//	sasfit_out("r2b: %lf\n", r2b);
    if ( param->errStatus ) return 0.0;
...
//	sasfit_out ("r6b: %lf\n", dom_cap_left_imag(param));
    r7a  = sasfit_integrate( (R_SH+T_SH)*cos(BIG_THETA_1), R_1 + D_1,
                             outer_curve_right_real, param );
//	sasfit_out("r7a: %lf\n", r7a);
    if ( param->errStatus ) return 0.0;
    r7b  = sasfit_integrate( (R_SH+T_SH)*cos(BIG_THETA_1), R_1 + D_1,
                             outer_curve_right_imag, param );
//	sasfit_out("r7b: %lf\n", r7b);
    if ( param->errStatus ) return 0.0;
    r8a  = sasfit_integrate(  -D_2 - R_2, (-1)*(R_SH+T_SH)*cos(BIG_THETA_2),
                              outer_curve_left_real, param );
//	sasfit_out("r8a: %lf\n", r8a);
    if ( param->errStatus ) return 0.0;
    r8b  = sasfit_integrate( -D_2 - R_2, (-1)*(R_SH+T_SH)*cos(BIG_THETA_2),
                             outer_curve_left_imag, param );
//	sasfit_out("r8b: %lf\n", r8b);
    if ( param->errStatus ) return 0.0;
    r9a  = sasfit_integrate( ((R_SH+T_SH)*cos(BIG_THETA_1)) - T_C, R_1 + D_1 - T_C,
                             inner_curve_right_real, param );
//	sasfit_out("r9a: %lf\n", r9a);
    if ( param->errStatus ) return 0.0;
    r9b  = sasfit_integrate( ((R_SH+T_SH)*cos(BIG_THETA_1)) - T_C, R_1 + D_1 - T_C,
                             inner_curve_right_imag, param );
//	sasfit_out("r9b: %lf\n", r9b);
    if ( param->errStatus ) return 0.0;
    r10a = sasfit_integrate( -D_2 + T_C - R_2,
                             ((-1)*(R_SH+T_SH)*cos(BIG_THETA_2)) + T_C, inner_curve_left_real, param );
//	sasfit_out("r10a: %lf\n", r10a);
    if ( param->errStatus ) return 0.0;
    r10b = sasfit_integrate( -D_2 + T_C - R_2,
                             ((-1)*(R_SH+T_SH)*cos(BIG_THETA_2)) + T_C, inner_curve_left_imag, param );
//	sasfit_out("r10b: %lf\n", r10b);
    if ( param->errStatus ) return 0.0;
// Adding the contributions to the amplitude (real parts and imaginary parts separately): (2) - (1) - (3) - (4) + (5) + (6) + (7) + (8) - (9) - (10)
    real_amp	= r2a - r1a - cap_right_real(param) - cap_left_real(
                      param) + dom_cap_right_real(param) + dom_cap_left_real(param) + r7a + r8a - r9a
                  - r10a;
    imag_amp	= r2b - r1b - cap_right_imag(param) - cap_left_imag(
                      param) + dom_cap_right_imag(param) + dom_cap_left_imag(param) + r7b + r8b - r9b
                  - r10b;
...
double sasfit_ff_magneticfieldalignedvesicle_psi_new_f(double q,
        sasfit_param * param)
{
// insert your code here
    return 0.0;
}
double sasfit_ff_magneticfieldalignedvesicle_psi_new_v(double q,
